...
func (i *RabbitMQInvalidator) PublishInvalidation(ctx context.Context, signal CacheInvalidationSignal) error {
	...
	// This is the same broadcast.Exchange leaflab/broadcast declares for every leaflab
	// cross-process signal (see leaflab/processor/ack_signal.go for the other one, FR47).
	routingKey := broadcast.RoutingKeyCacheInvalidation
...
func (l *LocalCacheInvalidationListener) ListenAndInvalidate(ctx context.Context) error {
	...
	l.consumer.RegisterHandler(broadcast.RoutingKeyCacheInvalidation, handler)
