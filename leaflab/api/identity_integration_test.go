...
// board's household up front (FR1.2/FR1.3, #1339) before reaching FR17's
// identity check below it, so every real-push-path test in this file needs
// a working (if trivial) authzResolver even though none of them exercise
// household scoping itself -- identitySchema carries no household concept
// at all, so every board resolves as Unclaimed.
func newIdentityTestServer(t *testing.T) (*LeafLabAPIServer, *pgxpool.Pool) {
	...
	return NewLeafLabAPIServer(repo, unclaimedAuthz{}, nil, nil, nil, nil, discardLogger()), db.Pool
}
