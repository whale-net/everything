...
func TestInsertDeviceConfigNextVersion_CommitsWriteAndAuditRowTogether(t *testing.T) {
	...
	version, err := repo.InsertDeviceConfigNextVersion(ctx, boardID, []byte(`{}`), nil, entry)
	if err != nil {
...
func TestInsertDeviceConfigNextVersion_FailedWriteLeavesNoAuditRow(t *testing.T) {
	...
	_, err := repo.InsertDeviceConfigNextVersion(ctx, neverInsertedBoardID, []byte(`{}`), nil, testAuditEntry())
	if err == nil {
