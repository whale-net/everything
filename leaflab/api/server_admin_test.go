...
func TestRequireAdminEligible_RefusesNonAdmin_AllFiveRPCs(t *testing.T) {
	...
	t.Run("ResolveToHousehold", func(t *testing.T) {
		repo := &fakeRepo{}
		server := NewLeafLabAPIServer(repo, &fakeAuthz{}, nil, nil, nil, nil, discardLogger())
...
func TestElevate_ConfigurableDuration(t *testing.T) {
	...
	server := NewLeafLabAPIServer(repo, &fakeAuthz{}, nil, nil, nil, nil, discardLogger(), WithElevationDuration(customDuration))
...
func TestGetDeviceConfig_AdminNoElevation_RefusedSameAsNonexistent(t *testing.T) {
	...
	server := NewLeafLabAPIServer(repo, az, nil, nil, nil, nil, discardLogger())
