...
func TestElevate_ConfigurableDuration(t *testing.T) {
	...
	server := NewLeafLabAPIServer(repo, &fakeAuthz{}, nil, nil, discardLogger(), claim.DefaultConfig, ratelimit.NewInMemoryLimiter(nil), WithElevationDuration(customDuration))
...
func TestGetDeviceConfig_AdminNoElevation_RefusedSameAsNonexistent(t *testing.T) {
	...
	server := NewLeafLabAPIServer(repo, az, nil, nil, discardLogger(), claim.DefaultConfig, ratelimit.NewInMemoryLimiter(nil))
