...
| `auth` | OAuth2 authorization-server front end for MCP servers — RFC 9728/8414 discovery metadata, RFC 7591 dynamic client registration, and a DB-backed mint/verify/revoke/list lifecycle for the opaque bearer credential it issues; the caller's identity comes from a pluggable, already-established-session `CallerResolver`, never a fresh verification against an external IdP; schema is applied by the consuming domain's own migration | [README](go/auth/README.md) |
| `mcpobs` | Shared MCP tracing/logging middleware — server-side per-tool-call span/log wrapper (each call its own trace, since the transport roots tool contexts at the session's `initialize` request) and client-side trace-propagating transport wrap | [README](go/mcpobs/README.md) |
| `migrate` | Postgres schema migration CLI/library (history tracking, rollback detection) | [README](go/migrate/README.md) |
| `monty` | Go client for the Monty Python sandbox — runs untrusted snippets in a crash-isolated worker and returns values, printed output and Python exceptions; one API over two transports, a pooled local `monty subprocess` child and a `monty-server` WebSocket in `monty/remote` (separate `go_library` so the core carries no WebSocket dependency, same divergent-BUILD-shape pattern as `grpcauth/pgstore`'s row above); speaks Monty's public protobuf parent/worker schema, vendored at `monty/protos` | [README](go/monty/README.md), [DESIGN](go/monty/DESIGN.md) |
