...
func TestMigration036_AppliesOnTopOfFullHistoryAndCreatesExpectedShape(t *testing.T) {
	...
	if latest != 36 {
		t.Fatalf("expected the latest migration source version to be 36, got %d -- update this test if a newer migration has since landed", latest)
	}
	...
	if version != 36 {
		t.Fatalf("expected version 36 after Up, got %d", version)
	}
