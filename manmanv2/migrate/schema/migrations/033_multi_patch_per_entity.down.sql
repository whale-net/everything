-- Drop views (CASCADE removes triggers too)
...
-- Remove patch_order column
ALTER TABLE configuration_patches
    DROP COLUMN patch_order;

-- Re-add unique constraint (fails if duplicates exist — caller must clean up first)
ALTER TABLE configuration_patches
    ADD CONSTRAINT configuration_patches_strategy_id_patch_level_entity_id_key
    UNIQUE(strategy_id, patch_level, entity_id);

-- Recreate covering indexes without patch_order (016-era state)
...
-- Recreate INSTEAD OF triggers (016-era, without patch_order)
