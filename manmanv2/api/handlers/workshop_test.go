...
func (m *MockWorkshopManager) InstallAddon(ctx context.Context, sgcID, addonID int64, forceReinstall, skipDispatch bool) (*manman.WorkshopInstallation, error) {
	args := m.Called(ctx, sgcID, addonID, forceReinstall, skipDispatch)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*manman.WorkshopInstallation), args.Error(1)
