...
- [docs/oidc_link_e2e_verification.md](docs/oidc_link_e2e_verification.md) — The end-to-end verification runbook for the delegated identity-link flow: prerequisites, the one-time link prompt, the browser OIDC login, and the follow-up session's recorded delegated subject. **Written but not yet executed** — the live run needs a human with Slack and Keycloak access. Read this before touching `WHAGENT_ON_BEHALF_OF_ALLOWED_CLIENT_IDS`, which ships empty on purpose.
