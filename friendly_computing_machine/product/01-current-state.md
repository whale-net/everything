...
- **manman V1 subscribe** (live, V1 only). `bot/subscribe/service.py` consumes `fcm-{env}.manman.generic.status` on the V1 `external_service_events` exchange. It upserts `manmanstatusupdate` and posts or edits Slack messages. The target channel type is hardcoded to `"manman_dev"`.
