...
- **Ad-hoc polls** (live). `/poll` in `bot/handlers/poll.py` posts a Simple Poll-style Block Kit poll. Vote and close buttons arrive over Socket Mode; each vote is written to `pollvote` and the message is re-rendered with `chat.update`. See `docs/poll.md`.
