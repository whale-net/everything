...
- C12 — A community member can start an ad-hoc poll with `/poll` (Simple Poll-style: quoted question and options, optional `anonymous` and `limit N`), vote with buttons that update the message live, and trust that every vote is kept in FCM's store.
