...
LB3 — Prediction-vs-outcome record shape (idea → verdict → schedule → published video → metrics)
  At risk: C10 (Now — browsing past comparisons) and C14 (Next — calibration trends across many ideas).
  Decide now: give the viability verdict a stable identity that the schedule draft, the committed schedule entry, and the eventual synced video metrics all reference by foreign key through the whole chain — not a denormalized "verdict text copied onto the schedule row" shortcut. C14's calibration-trend view is an aggregate query over exactly this chain; if idea identity is lost or duplicated anywhere between verdict and outcome, that aggregate requires a backfill/reconciliation pass instead of a `GROUP BY`.
...
LB5 — Web/MCP dual-surface parity mechanism
  At risk: C4, C5, C6, C7, C9, C10, C14 (each becomes dual-surface as its own M4.x milestone ships), and NFR3's web-minimization intent (`ARCHITECTURE.md`'s "NFR3 interface allocation" section).
  Decide now: every M4.x web surface must call the identical store method and identical `store.CanX` authorization check its MCP-tool counterpart already uses — never a parallel or divergent write/read path. This is the same "two independent, equally-capable front ends onto the same store, not a primary and a read-only shadow" pattern `ARCHITECTURE.md`'s NFR3 amendment already established for C8 (issue #1648) and C11/C12/C13 (issue #1728); M4 generalizes it to the remaining MCP-only capabilities. C4/C5's web surface is scoped to save + browse only — the persisted note/verdict CRUD and its rendering, the same objects `save_research_note`/`save_viability_verdict` already write — never a chat/reasoning surface: the research/viability reasoning conversation itself stays an external MCP-capable agent's job, per the standing no-hosted-agent-loop non-goal. Supersession of NFR3 happens per-milestone-as-it-ships, not blanket and not present-tense: NFR3 continues to list all seven as MCP-only until each M4.x milestone's own `/project-manager:design --milestone` pass appends its own `**NFR3 amendment (issue #<M4.x root plan issue>): <capabilities> are dual-surface.**` paragraph to `ARCHITECTURE.md`'s "NFR3 interface allocation" section — mirroring the #1648 and #1728 paragraphs already there (same heading shape, same "which store method / which `CanX` check" content) — before that milestone is considered designed. This is a requirement, not a suggestion: a milestone is not done reallocating interface until its own paragraph exists. Separately, a browser save-form's retry model (double-submit on back-button/refresh) differs from LB4's original agent-tool-call-retry framing; the shared store method this LB requires should inherit LB4's contract rather than needing its own — to be confirmed, not assumed, in each milestone's design pass.
  Stays cheap: which pages exist, their layout/UX, and the M4.1/M4.2/M4.3 phase ordering itself — all additive once every write/read path is already routed through the shared store+authz layer.
...
| [`product/02-capability-map.md`](product/02-capability-map.md) | Capability map — C1..C16, bucketed Now/Next/Later |
| [`product/03-roadmap.md`](product/03-roadmap.md) | Milestone definitions — M1, M2, M3, M4.1, M4.2, M4.3 |
