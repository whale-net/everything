...
5. **Stop at the draft.** Turning a draft into the Channel's committed schedule (C8) is a Creator-only action taken in the `web` UI (NFR3's interface allocation) — there is no MCP tool for it, and this is deliberate, not a gap. Tell the human the draft is ready for the Creator to approve in `web`; never claim you approved or committed it.
