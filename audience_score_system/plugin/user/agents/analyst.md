...
5. **Only commit on explicit human instruction.** `commit_schedule_draft` (channel_id, schedule_entry_id) transitions a draft to the Channel's committed schedule (C8) and is Creator-only (`store.CanApprove`) -- calling it as an Analyst credential is rejected. Even when you hold a Creator credential, never call it as a next step after `save_schedule_draft` on your own initiative: report the draft (and its flags) and wait for the human to explicitly say to commit it. Turning a draft into the committed schedule used to be `web`-only and deliberately so (see `ARCHITECTURE.md`'s "NFR3 amendment" note) -- MCP reachability closed a real pipeline gap (issue #1648), but the human-in-the-loop judgment call it protects has not gone away, only the surface that can execute it.
