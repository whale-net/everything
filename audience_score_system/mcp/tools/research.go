...
// name and relations) as ResearchNoteOutput. Cited is derived by
// n.Cited() (FR10, FR12 -- store.ResearchNote.Cited, models.go) at this
// single call site so save_research_note and list_research_notes can
// never disagree on the rule. relations is this note's own
// store.NoteRelation slice (issue #1942, FR11) -- callers batch-resolve
// it via store.ResearchStore.ListRelationsForNotes ONCE per response and
// pass in just this note's entry (nil is fine: it renders as an absent
// Relations field, never an empty array).
...
func registerListResearchNotes(reg *server.Registry, research store.ResearchStore) {
	server.RegisterRead(reg, &mcp.Tool{
		...
		Description: "List research notes for a Channel, most-recent first, each carrying an explicit cited boolean " +
			"(FR10) and its own relations field (FR11) -- both directions: outgoing edges this note declared, and " +
			"incoming edges another note declared pointing at it (an incoming supersedes/excludes means THIS note " +
			"is retired). Optionally restrict to one Idea (idea_id) and/or partition into cited_only vs uncited_only. " +
