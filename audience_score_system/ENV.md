...
| `ASS_GOOGLE_CLIENT_ID` | web, worker | *(required)* | Google OAuth2 client ID (Google Cloud Console "OAuth client ID") for the sign-in consent screen. `worker` reads the same variable to refresh a Channel's C2 credential (see "Temporal" above, issue #1576) — it never runs the sign-in flow itself. `audience_score_system/Tiltfile` defaults this to a fixed dummy client id locally (never authenticates against real Google infrastructure), overridable via a local `.env`. |
| `ASS_GOOGLE_CLIENT_SECRET` | web, worker | *(required)* | Google OAuth2 client secret paired with `ASS_GOOGLE_CLIENT_ID`. Same worker note as above. `audience_score_system/Tiltfile` local-dev default note also applies. |
| `ASS_OAUTH_REDIRECT_BASE_URL` | web, mcp | *(required)* | This app's own externally-reachable origin, e.g. `https://audience-score.whalenet.dev` — `web/auth` appends `/oauth/google/callback` to build the redirect URL Google calls back to. **Also doubles as the OAuth2 issuer** (issue #1646, FR12/NFR4): `web` passes it as `auth.ProviderConfig.Issuer`, so it is the base every `auth` endpoint URL `web` advertises (`/authorize`, `/token`, `/register`, `/.well-known/oauth-authorization-server`) is built from — one origin, two roles (Google's redirect target and auth's issuer), not two variables. `mcp` reads the SAME value (never calling it) purely to name it as the sole `authorization_servers` entry in its own protected-resource metadata (see "MCP" below) — `web` and `mcp` MUST agree on this exactly, or an MCP client's discovery chain (RFC 9728 → RFC 8414) points at the wrong authorization server. |
| `ASS_SESSION_SECRET` | web | *(required)* | Secret used to sign/protect the short-lived OAuth2 state cookie (CSRF) during the login round trip. `audience_score_system/Tiltfile` defaults this to a fixed `dev-secret-key` placeholder locally, overridable via a local `.env`. |
| `ASS_TOKEN_ENCRYPTION_KEY` | web, worker | *(required)* | Secret hashed (SHA-256) into a 32-byte AES-256-GCM key used to encrypt any stored Google refresh token at rest — mirrors `libs/go/htmxauth.DBSessionManager`'s `encKey` derivation. This same key also encrypts `channel_credential`'s YouTube token ciphertext (see "OAuth scopes" below) — one key, two independent token stores. `worker` derives the same key to decrypt/refresh `channel_credential` for `SyncSchedule` (issue #1576). `audience_score_system/Tiltfile` local-dev default note also applies (same `dev-secret-key` placeholder). |
| `ASS_WHAGENT_UI_JWKS_URL` | web | *(required for the link flow)* | `whagent_net/ui`'s own JWKS endpoint (`{WHAGENT_UI_PUBLIC_URL}/.well-known/jwks.json`), passed to `link.NewVerifier` (issue #2598, FR3) so `web` can verify a `ui`-minted link assertion (`audience_score_system/web/link`). This is a **distinct pair from `ASS_WHAGENT_JWKS_URL`/`ASS_WHAGENT_ISSUER`** above — that pair verifies `api`-minted persona `Claim`s on `mcp`; this pair verifies `ui`-minted link assertions on `web`, points at `ui`'s own JWKS endpoint, and checks against `ui`'s own issuer value. Never reused, aliased, defaulted from, or falling back to the `mcp` pair, or vice versa. `audience_score_system/Tiltfile` defaults this to `whagent_net`'s in-cluster `ui` Service address locally, overridable via `BUILD_WHAGENT_ENV`/`ASS_WHAGENT_UI_JWKS_URL`. |
| `ASS_WHAGENT_UI_ISSUER` | web | *(required for the link flow)* | `ui`'s own `iss` value (= `WHAGENT_UI_PUBLIC_URL`) — `link.Verifier` rejects any assertion whose `iss` doesn't match this exactly. See `ASS_WHAGENT_UI_JWKS_URL` above for the independence from `mcp`'s `ASS_WHAGENT_ISSUER`. `audience_score_system/Tiltfile` defaults this to `http://localhost:8081` — it MUST stay equal to `whagent_net/Tiltfile`'s `ui` block's `WHAGENT_UI_PUBLIC_URL` if either changes. |
...
(migration 006, backed by `libs/go/auth.CredentialStore`) stores only a
SHA-256 hash, not a reversible secret. `auth.NewCredentialStore`
...
| `ASS_MCP_PUBLIC_URL` | web, mcp | *(required)* | The externally reachable URL of the `mcp` server (issue #1646, FR12/NFR4) — the OAuth2 `resource` identifier both binaries must agree on exactly. `web` passes it as `auth.ProviderConfig.Resource` when constructing its OAuth2 authorization server (see "Web" above); `mcp` passes the same value as `auth.ProtectedResourceMetadataConfig.Resource` (`mcp/server.ResourceMetadataConfig.Resource`) — both the `resource` field its own `/.well-known/oauth-protected-resource` document advertises, and the base its `WWW-Authenticate: Bearer resource_metadata="..."` 401 challenge is built from (`auth.ProtectedResourceMetadataURL`). A mismatch between the two binaries breaks MCP client discovery (RFC 9728), since the `resource` an MCP client requests a token for would no longer match the `resource` `mcp` actually serves. Both `web` and `mcp` fail fast at startup if unset. It also doubles as the `aud` (audience) every whagent-net Claim must carry (see `ASS_WHAGENT_ISSUER` below) — no separate audience variable is introduced. In any real deployment leave `ASS_WEB_MCP_RESOURCE_URL` below unset so `web` and `mcp` keep agreeing on this one exact value. |
| `ASS_WEB_MCP_RESOURCE_URL` | web | *(optional, falls back to `ASS_MCP_PUBLIC_URL`)* | Local-dev-only override of the `Resource` value `web` passes to `auth.NewProvider` (issue #2643). `ASS_MCP_PUBLIC_URL` is normally an in-cluster-DNS or public `https://` address; `libs/go/auth`'s `validateAbsoluteURL` rejects a non-`https`, non-loopback URL, so a stock Tilt dev cluster's in-cluster-DNS default crash-loops `web`. Setting this to the loopback address `mcp`'s own `port_forwards` exposes (e.g. `http://localhost:8084/`) lets `web` satisfy that check while `mcp`/`worker` keep reading `ASS_MCP_PUBLIC_URL` unchanged. Never set outside local dev. |
...
| Server | Connection |
|---|---|
| `audience-score-system-pg-tilt` | Hardcoded to the local default (`postgres://postgres:password@localhost:5433/audience_score_system`) — not a secret |
