...
| `ASS_WEB_MCP_RESOURCE_URL` | web | *(optional, falls back to `ASS_MCP_PUBLIC_URL`)* | Local-dev-only override of the `Resource` value `web` passes to `mcpauth.NewProvider` (issue #2643). `ASS_MCP_PUBLIC_URL` is normally an in-cluster-DNS or public `https://` address; `libs/go/mcpauth`'s `validateAbsoluteURL` rejects a non-`https`, non-loopback URL, so a stock Tilt dev cluster's in-cluster-DNS default crash-loops `web`. Setting this to the loopback address `mcp`'s own `port_forwards` exposes (e.g. `http://localhost:8084/`) lets `web` satisfy that check while `mcp`/`worker` keep reading `ASS_MCP_PUBLIC_URL` unchanged. Never set outside local dev. |
