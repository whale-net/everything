...
| `mergepush` | Dispatched once per batch by `implement`/`validate` after worker/validator return: removes each finished task's worktree, then registers its branch into the plan's one real `gh stack` stack via `gh stack link` — pushes it, opens/refreshes its PR, and corrects the PR base so the plan stays one reviewable stack (`main → task-a → task-b → ...`) instead of a tree, without needing to touch the branch's own git history — sets title/body on first creation, then merges into `main` whatever's now `Done` and ready, continuously, rather than holding the whole stack open for one atomic merge at the end. Keeps the orchestrator's own context free of `git`/`gh stack` command output | haiku |
