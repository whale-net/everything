...
| `worker` | Executes tasks in `Scaffold`, `Implementation`, and `Testing` swimlanes inside a dedicated worktree, commits to the task's own branch, and advances tasks to the next swimlane | haiku |
...
| `mergepush` | Dispatched once per batch by `implement`/`validate` after worker/validator return: pushes each finished task's branch as-is, opens or refreshes its own plain PR based on its real `Depends on:` dependency — sets title/body on first creation — then merges into `main`, one task at a time in dependency order, whatever's now `Done` and ready, continuously, rather than holding anything open for one atomic merge at the end; removes each task's worktree as best-effort cleanup. Keeps the orchestrator's own context free of `git`/`gh` command output | haiku |
                                              ...
                                              producer creates root Issue (plan:approved, or plan:agent-approved if reviewer approved it)
                                                        ...
                                                        /project-manager:plan ──dispatches──▶  planner  ──creates──▶  Project board with Swimlanes:
                                                                           ...
                                                                           (claim via assignee, advance Status, unassign / close when Done)
                                                                                        │
                                                                                        ▼
                                                                              mergepush (push branch, open/refresh its own PR, merge when Done — once per batch)
                                                                                        │
                                                                                        ▼
...
| `/project-manager:implement <issue-number> [--max-subagents N]` | Orchestrates worker/validator subagents in parallel batches — up to `--max-subagents` (default 4) at a time — each in its own dedicated worktree from branch creation onward, over per-task branches based on `main` or their real `Depends on:` dependency, until all tasks are `Done`; each batch's push/PR integration and continuous trunk-merge is handed to `mergepush`, which pushes every task's branch, opens/refreshes its own plain PR, and merges into `main` (one task at a time, in dependency order) whatever's `Done` and ready every batch, so most tasks land individually as they validate instead of at the very end — the orchestrator's own session stays free of `git`/`gh` output either way. Requires a Project board to already exist | `worker`, `validator`, `mergepush` |
| `/project-manager:validate <issue-number>` | Whole-system validation in Tilt (against a local branch merging every task's tip together, since not every task's work is guaranteed to already be combined on any one branch) once all tasks are `Done`; lands whatever continuous merge during `/project-manager:implement` hadn't already merged once validation passes, or routes findings to planner | `system-validator`, `planner` |
