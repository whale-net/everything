...
1. `gh issue view <n> --comments` — report the issue's title and status:
   - If labeled `product:approved`, this is a product tracking issue, not a plan. Read `<domain>/PRODUCT.md` (path from the issue's first line) from `main`, follow its jump table to `product/03-roadmap.md` for the milestone list, then reconstruct each milestone's status from the comments already fetched: take the **last** `Ledger: M<n> → <status> (<link>)` comment per milestone (no comment means `not started` — CONVENTIONS.md § Roadmap ledger). Report milestone, status, and the link each ledger comment carried. Also reconstruct, per slug, the last `Amendment: <slug> → <status> (<link>)` comment; report any still sitting at `adopted-but-pending-amendment` (CONVENTIONS.md § Out-of-band adoptions) as outstanding, and flag it explicitly once 3 or more are outstanding — that count alone is grounds to run `/project-manager:product <n>` and fold them in. Recurse into step 1 for each milestone that has a root plan issue so the user sees a one-line state per milestone. Name the next action: `/project-manager:design <n> --milestone M<k>` for the first `not started` milestone, or the appropriate skill for whichever milestone is furthest along. Stop here — the rest of these steps are per-plan.
   - If labeled `plan:approved` or `plan:agent-approved` (CONVENTIONS.md § Agent-approved plans — functionally identical here; report which one it is) with a `Project board: <url>` comment, proceed to step 2. If its body's first line names a product brief (`Product: #<p> — Milestone M<k>`), report that too.
   - If labeled `plan:approved` or `plan:agent-approved` without a `Project board:` comment, report that task breakdown hasn't started yet and `/project-manager:plan <n>` is next.
