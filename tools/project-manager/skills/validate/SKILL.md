...
4. **If everything passed:** Finalize per CONVENTIONS.md § Git hygiene step 8:
   - Make sure every task's PR is open and merged: dispatch `mergepush` once more with `<n>`, the full `<plan-branches>` list (every task branch on this plan, in dependency order — reconstruct it the same way `implement`'s "Task PRs" section does if this session didn't just run `/project-manager:implement` itself), an empty batch-to-integrate list, and `<done-task-numbers>` set to every task on the plan (step 1 already confirmed all are `Done`) — pushing/opening a PR is a no-op re-run for anything already pushed, and the merge part lands whatever `mergepush`'s continuous-merge attempts during `/project-manager:implement` hadn't already landed. In the common case where continuous merge already landed everything, this call merges nothing and just confirms it.
   - Check `gh pr view <branch> --json number,state` for every entry in `<plan-branches>` — if any isn't `MERGED` yet after the `mergepush` call above, something's still blocking it (a failing check, a stale approval); resolve that before considering the plan closed out, same as any other merge failure (CONVENTIONS.md § Git hygiene, "Division of responsibility").
   - Collect every task's PR URL — prefer the URLs already gathered during `/project-manager:implement`; otherwise fetch them all in one `gh pr list` call (CONVENTIONS.md § Git hygiene step 8), not one `gh pr view` per branch.
