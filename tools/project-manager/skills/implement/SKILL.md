...
Drives everything from `plan:approved` through worker execution. See `tools/project-manager/CONVENTIONS.md` § Project setup, § Task issues, and § Worker lifecycle for the contract this dispatches against.
...
2. **Task breakdown (skip if already done).** `gh issue view <n> --comments` — if a `Project board: <url>` comment exists, the plan's Project already exists; extract its number and skip to step 3. Otherwise dispatch `project-manager:planner` (foreground, via Agent tool) with the root issue number to run its Process: create the Project, break the plan into scaffold/implementation/testing/validation task issues with dependencies, add them to the Project with the right `Status`, and post the summary + `Project board:` comment.
