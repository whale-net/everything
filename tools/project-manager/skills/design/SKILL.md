...
1. **Resolve the discussion.**
   - **Check `--milestone` first** — with it, the positional argument is a product issue number, not a discussion. `gh issue view <n>` and confirm the `product:approved` label; its body's first line points at `<domain>/PRODUCT.md`. Read that file from `main` — if it isn't there yet, the docs PR from `/project-manager:product`'s publish step hasn't merged; tell the user to merge it first and stop. Find the milestone in its roadmap; if it is absent, list the ones that exist and stop. `gh issue view <n> --comments` and take the **last** `Ledger: M<n> → <status> (<link>)` comment for this milestone (CONVENTIONS.md § Roadmap ledger) — already `in design` or later means that comment carries a discussion URL, so resume that discussion instead of opening a second one; no such comment means `not started`. Warn (don't block) if an earlier milestone is still `not started`; milestones are ordered so each builds on the last, and designing out of order is occasionally right but usually a mistake.
