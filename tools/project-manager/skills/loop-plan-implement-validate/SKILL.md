...
6. **Final verification (subagent).** Dispatch one more fresh `general-purpose` subagent — never inline in this session. Hand it *only* the `{task issue → branch → PR number/URL}` description already collected across steps 3-4 (the running `<plan-branches>` from the last `implement`/`validate` reports), not an open-ended mandate to go rediscover the plan's state itself. Ask it to independently confirm, using that description as its worklist:
