...
| `--resume-agents` | off | Resume the same producer/architect subagent for a follow-up round (steps 5-7) instead of spawning a fresh one — avoids re-grounding on the whole Discussion each round. Does not reach the amendment loop (step 8), which is always a separate invocation. See CONVENTIONS.md § Resume mode. |
...
4. **Draft the brief.** Dispatch `project-manager:producer` (Mode P1) with the intake transcript and discussion URL and an explicit `name: "producer-<discussion-number>"` (and, for architect once it's dispatched in step 5, `name: "architect-<discussion-number>"`) so a later round has a stable target to resume under `--resume-agents`: write the draft brief — vision, personas, capability map, non-goals — to the working-draft gist and post a short summary comment on the Discussion (CONVENTIONS.md § Working draft). No current state, no load-bearing decisions, no roadmap yet; those need architect first.
...
6. **Roadmap and loop.**
   ...
   - **With `--resume-agents`:** each redispatch in this loop targets the same `producer-<discussion-number>` / `architect-<discussion-number>` names from steps 4-5 via `SendMessage` instead of a fresh `Agent` call — see CONVENTIONS.md § Resume mode.
...
7. **Human gate and publish.** Present the signed-off brief to the user: the vision, the capability map bucketed, the load-bearing decisions, and the milestone list with what M1 does and does not include. Ask for approval, changes, or a re-cut of the milestone boundaries.
   - **Changes** — dispatch producer (Mode P2) and architect for another round in the discussion, then return here. **With `--resume-agents`:** target the same `producer-<discussion-number>` / `architect-<discussion-number>` names via `SendMessage` — this is still the same invocation (CONVENTIONS.md § Resume mode).
...
8. **Amendment (existing brief).** Reality changes roadmaps — a shipped milestone teaches you something, or a milestone's design surfaces a capability nobody had thought of. The spec is a living document, but never edited silently:
   - **First, check for outstanding out-of-band adoptions.** Reconstruct, per slug, the last `Amendment: <slug> → <status>` comment on the tracking issue; any still sitting at `adopted-but-pending-amendment` (CONVENTIONS.md § Out-of-band adoptions) is a hand-made change nobody has folded into the spec yet. Bundle every such slug into this amendment instead of drafting them separately — even if the amendment that brought you here started as something unrelated. If there is no unrelated trigger at all but 3 or more slugs are stuck at `adopted-but-pending-amendment`, that count is the trigger: run this step to fold them in on its own.
   - Dispatch producer (Mode P2) to draft the amendment as a comment on the **tracking issue** — not the original product discussion, which is closed once the brief is published — picking a short kebab-case slug and posting `Amendment: <slug> → drafted (<comment-url>)` right after, and architect to reconcile it there (posting `Amendment: <slug> → reconciled (<comment-url>)` alongside its sign-off, even when it finds no load-bearing/ordering impact) if it touches load-bearing decisions or milestone ordering, baselining against the committed `<domain>/product/01-current-state.md` on `main`. These `Amendment: <slug> → <status>` comments (CONVENTIONS.md § Amendment ledger) are what let a later reader tell drafted/reconciled/merged apart without parsing comment prose.
   - Present the diff to the user for approval.
   - On approval, producer repeats the publish mechanics but touches only the file(s) the amendment actually changes (`PRODUCT.md` for a load-bearing/non-goal change, `product/02-capability-map.md` for a new capability, `product/03-roadmap.md` for a re-cut milestone) — a fresh branch, an edit, a PR — never `gh issue edit` on the tracking issue's body. Once the PR merges, it comments `Amended: <summary> (#<pr-number>)` **and** `Amendment: <slug> → merged (#<pr-number>)` on the tracking issue — including one `merged` comment per out-of-band slug folded in this round.
