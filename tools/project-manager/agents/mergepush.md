...
This also means you never need to trust — or repair — `gh stack`'s own tracked trunk for a branch, only `git`. That matters for a plan whose tasks were created before this flattening existed, or whose earlier batches ran before it did: those branches may still be chained onto a sibling task's branch instead of the real shared dependency, and once this repo's `delete_branch_on_merge` setting deletes that sibling's branch after it merges, `gh stack`'s local trunk for the chained branch names a branch that no longer exists remotely (a legacy shape, but not a rare one — it recurs any time a shared dependency merges before a plan mid-flight finishes). You don't need to detect or repair any of that specially: rebasing onto whatever `<chain-tip>` the caller hands you (step 3) works regardless of what `gh stack` thinks that branch's trunk is. The caller is responsible for handing you tasks in real dependency order and a correct `<chain-tip>` to start from — including, for a plan resuming with no known tip, a one-time bootstrap pass over every still-open task branch (`implement` § "Chain tip" — "More than one such leaf turns up").
