...
| No human reviewer available for this plan right now, or the requester explicitly wants the whole design phase run unattended | `loop-design-panel "<feature>"` or `loop-design-panel <discussion-url>` | Runs `design --stakeholder-meeting` plus a `reviewer` subagent that stands in for the human at both the stakeholder round cap and the final review decision (CONVENTIONS.md § Agent-approved plans). Creates the root Issue labeled `plan:agent-approved`, not `plan:approved` — functionally identical downstream, but distinguishable. Not for a requester who wants a human to actually look at this plan. |
| Root Issue is `plan:approved` or `plan:agent-approved`, no Project board yet | `plan <issue-number>` | Also the answer to "just set up the board" / "plan only" without starting execution. Both labels gate identically. |
...
| `plan:approved` or `plan:agent-approved`, and the requester wants the whole thing driven to a merged stack without re-running each phase by hand | `loop-plan-implement-validate <issue-number>` | Chains `plan`→`implement`→`validate` in subagents, re-looping `implement`↔`validate` on findings, until the stack is merged. Not for a requester who wants to inspect/steer each phase themselves. |
