...
Task work is tracked with [`gh stack`](../../.claude/skills/gh-stack/SKILL.md) — each task issue gets its own branch and, once pushed, its own small reviewable PR, instead of every task piling commits onto one shared branch. Only `/project-manager:implement`, `/project-manager:validate` (for the final integration branch), and the `mergepush` persona they dispatch run `gh stack`/branch-management commands; `worker` and `validator` receive an already-created worktree and branch, and never touch git branch/stack state themselves — this keeps `gh stack` operations single-threaded even though multiple worker/validator subagents run concurrently (its stack file is protected by a lock that times out after 5s — see gh-stack skill § Exit codes, code 8 — concurrent callers would just fight it). `mergepush` doesn't change that guarantee: the orchestrator dispatches it once per batch and waits for it to finish before scanning for the next one, so only one `gh stack` caller is ever active at a time — the git/gh-stack commands have just moved into a subagent's own context instead of running inline in the orchestrator's.

**Division of responsibility.** The orchestrator (`implement`/`validate`) owns every step below end to end, but doesn't necessarily run each one itself: it creates branches and worktrees and dispatches worker/validator subagents into them directly, resolves any merge conflict that comes up while doing so, and delegates flattening each task onto the plan's running chain tip and creating/refreshing its PR (step 6) to `mergepush`, dispatched once per batch of finished worker/validator results. The orchestrator carries the chain tip (a branch name, or `main` if nothing has been integrated yet) as a session variable across batches, seeding each `mergepush` dispatch with it and updating it from what that dispatch reports back. Worker and validator subagents only ever write code inside the worktree path they're handed — they never run `git merge`, `gh stack`, or open a PR themselves; `mergepush` never writes code — it only pushes commits worker/validator already made and manages the resulting PR. When a conflict is small (a handful of hunks, mechanically resolvable), the orchestrator resolves it inline and continues. When a conflict is large or semantically ambiguous (overlapping logic from two different tasks, unclear which side should win), the orchestrator dispatches an ad-hoc `general-purpose` subagent scoped to just that one merge — hand it the two branch tips, the conflicting files, and both tasks' issue descriptions for context — rather than resolving it blind or stalling the whole batch on it. The same escalation applies to a push `mergepush` reports as failed (a real rebase conflict or a lock timeout) — it stops on that one task and reports the error rather than guessing at a fix.
...
6. **After a subagent finishes** — this step is `mergepush`'s job, not the orchestrator's own; `implement`/`validate` dispatch it once per batch of finished worker/validator results (CONVENTIONS.md § Division of responsibility above) and wait for it to return before continuing. Step 3 above chains each task's branch onto its own `Depends on:` dependency (or `main`), which produces a *tree* whenever two tasks depend on the same third one — but the plan's PR history must stay one flat, strictly linear chain (`main → task-a → task-b → ...`, one parent and one child per branch) so it stays reviewable and mergeable in order. `mergepush` is what enforces that, re-parenting each task onto wherever the *previous* task landed rather than its original dependency:
   ```sh
   git worktree remove .pm-worktrees/<task-issue-number>
   git checkout pm-<root-issue-number>/<task-issue-number>-<slug>
   git fetch origin <current-tip>
   git rebase --rebase-merges origin/<current-tip>
   git push --force-with-lease origin pm-<root-issue-number>/<task-issue-number>-<slug>
   gh stack submit --auto
   gh pr edit <number> --base <current-tip>
   git checkout --detach HEAD
   ```
   `<current-tip>` starts as `main` for a plan's very first integrated task, then becomes each successfully-integrated task's own branch name in turn — the orchestrator carries it as a session variable across batches, seeded from whatever the previous `mergepush` dispatch last reported back. Nothing persists it durably, so a session resuming a plan with no in-memory tip has to derive it fresh — see `implement` skill § "Chain tip" for the derivation, and its "More than one such leaf turns up" case for bootstrapping a plan whose tasks were never fully flattened (created before this flattening existed, or an earlier run was interrupted mid-batch). This is safe because a task is only ever picked up once every issue in its `Depends on:` line is `CLOSED` (§ Worker lifecycle step 1), so its true dependency was already integrated — and therefore already an ancestor of `<current-tip>` — before this rebase runs; the rebase just replays this task's own commits past whatever else landed in between, never dropping or duplicating a real dependency's commits. `--rebase-merges` is required, not optional: a task with more than one dependency merged its extras in via `git merge --no-edit` in step 3 above, and a plain `git rebase` silently drops a merge commit's second parent instead of replaying it. The force-with-lease push is needed even on a branch pushed once already, since the rebase rewrites it. `submit` is idempotent — safe to run after every phase, not just once. It pushes and opens (or refreshes) the task's draft PR; the trailing `gh pr edit --base` then corrects the PR's base to `<current-tip>`, overriding whatever `submit` inferred from the branch's own separate tracked-at-creation base — this is what actually keeps the chain flat, and is harmless to re-run when the base was already correct. This is the pipeline's sole PR-creation path — no persona ever opens a PR directly with `gh pr create`.

   **PR content.** `submit --auto` generates the title and body from commit messages, which is rarely descriptive enough on its own (gh-stack skill § PR title auto-generation) — a task branch usually carries multiple phase commits (`scaffold:`, `feat:`, `test:`), which collapses the auto title down to just the humanized branch name. The first time `submit` creates a task's PR (not on later idempotent re-submits — `mergepush` checks for an existing PR with `gh pr view <branch> --json number,url` before submitting to tell the two apart), immediately follow up with `gh pr edit <number> --title "<title>" --body "<body>"`:
...
7. **Whole-system validation.** Because every task is already flattened onto one chain (step 6), the plan's current chain tip already *is* every integrated task's work, in order — there's no separate merge-fan-in needed. `/project-manager:validate` just branches off it, locally, right before dispatching system-validator, and never pushes it:
   ```sh
   git branch -D pm-<root-issue-number>-integration 2>/dev/null
   git fetch origin <chain-tip>
   git checkout -b pm-<root-issue-number>-integration origin/<chain-tip>
   ```
   where `<chain-tip>` is the branch name the last `mergepush` dispatch for this plan reported back (or `main`, if the plan has no integrated tasks — nothing to validate yet).
