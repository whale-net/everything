...
5. **Registering a task into the plan's stack** — this step is `mergepush`'s job, not the orchestrator's own; `implement`/`validate` dispatch it once per batch of finished worker/validator results (§ Division of responsibility above) and wait for it to return before continuing. Step 3 above chains each task's branch onto its own real `Depends on:` dependency (or `main`), which can fork into a *tree* whenever two tasks depend on the same third one — something `gh stack` itself can't represent as one stack (its stacks are strictly linear; see its skill § Known limitations #1). `mergepush` doesn't need to rewrite that tree in git to fix this: it grows the plan's *one* GitHub-tracked stack by appending each task, in the order tasks actually finish, via [`gh stack link`](../../.claude/skills/gh-stack/SKILL.md) — built exactly for growing a stack from already-existing branches without needing local `gh stack` tracking for any of them:
   ```sh
   git worktree remove .pm-worktrees/<task-issue-number>
   gh stack link <plan-branches...>
   ```
   `<plan-branches...>` is every branch belonging to this plan that's been registered into its stack so far (bottom to top, in the order they were previously registered), with this batch's newly-finished task branches appended at the end in the batch's own fixed processing order — not just the branch being registered right now. `link` is additive and idempotent: a branch already in the stack is skipped, so passing the whole list every time is harmless and never needs to be minimal, just correctly ordered and complete. On the very first call for a plan, this is a single branch and `link` creates a brand-new one-PR stack; every later call grows it. `link` pushes each new branch (non-force), creates its PR if none exists yet (or finds the existing one), and — this is what actually keeps the plan reviewable as a unit — corrects that PR's base to chain onto whatever is now below it in `<plan-branches...>`, regardless of the branch's own real dependency from step 3. No checkout, rebase, or force-push anywhere in this step: `link` operates on branch names and the remote directly, so it never touches the shared checkout, and it's safe to re-run.
...
6. **Whole-system validation.** `mergepush` only ever registers each task's *existing* branch into the plan's stack — it never rebases one task's content onto another's — so no single branch on the stack contains every task's combined work. `/project-manager:validate` builds one locally, right before dispatching system-validator, and never pushes it:
   ```sh
...
7. **Closing out.** The deliverable is the plan's one stack of small PRs, reviewed and merged together — not N separately-merged PRs, and not one big PR. Once validation passes, `/project-manager:validate` merges the whole stack atomically:
   ```sh
   gh stack merge <top-task-pr-number> --yes
   ```
   `<top-task-pr-number>` is the PR number of the last task registered into the stack (`<plan-branches...>`'s final entry from step 5) — `gh stack merge` accepts a PR number directly and merges everything below it too, so the whole stack lands in one all-or-nothing operation: if any PR can't merge (a failing check, a stale approval), none are. Pick a merge method with `--squash`, `--rebase`, or `--merge` if the plan needs something other than whatever method was last used (gh-stack skill § Agent rules, rule 10). Then post `gh issue comment <root-issue-number> --body "PRs: <url>, <url>, ..."` on the root issue, listing every task's PR (`gh pr view <branch> --json number,url` per branch, or the URLs already gathered during `/project-manager:implement`).
