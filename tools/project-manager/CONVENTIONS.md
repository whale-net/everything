...
Off by default. `/project-manager:design --resume-agents` and `/project-manager:product --resume-agents` change how the **orchestrator** redispatches the same persona for another round. Every default-mode round after the first re-reads the working-draft gist and the discussion's round comments — and often the domain docs — from a cold start, because a fresh `Agent()` call has no memory of the round before it; the gist convention (§ Working draft) keeps that read bounded, but it's still a cold read every round. Resuming the same subagent instance instead means it only needs the delta.
...
Each task issue gets its own branch and, once pushed, its own small reviewable PR based directly on its real `Depends on:` dependency (or `main`), so it reviews as a diff against exactly the trunk state it will land on. This pipeline is trunk-oriented: each task merges into `main` continuously, as soon as it's `Done` (validated) and every task it depends on is already on trunk (§ Task granularity & merge cadence, § Continuous merge to trunk below) — never held open as one unit merged atomically at the very end. Only `/project-manager:implement`, `/project-manager:validate` (for the final integration branch), and the `mergepush` persona they dispatch run `git push`/`gh pr` commands; `worker` and `validator` receive an already-created worktree and branch, and never touch git remote or PR state themselves. Branch creation (step 3) and `mergepush`'s push/PR step (step 5) both operate through a dedicated worktree or by branch name only — neither ever checks anything out in the shared checkout — so different tasks' branches can be created and pushed concurrently without racing each other; `mergepush` is still dispatched once per batch and waited on before the orchestrator scans for the next one, purely to keep its own commands out of the orchestrator's context, not because anything shared needs serializing.

**Division of responsibility.** The orchestrator (`implement`/`validate`) owns every step below end to end, but doesn't necessarily run each one itself: it creates a branch and worktree per task directly, resolves any merge conflict that comes up while doing so, and delegates pushing each finished task's branch, opening/refreshing its PR, and merging whatever's now ready (steps 5-6) to `mergepush`, dispatched once per batch of finished worker/validator results. The orchestrator is the one that knows each task's swimlane `Status`, so it also tells `mergepush` which branches are `Done` and eligible for step 6's merge — `mergepush` doesn't query the Project board itself. Worker and validator subagents only ever write code inside the worktree path they're handed — they never push, run `git merge`, or open a PR themselves; `mergepush` never writes code — it only pushes and merges commits worker/validator already made. When a conflict is small (a handful of hunks, mechanically resolvable), the orchestrator resolves it inline and continues. When a conflict is large or semantically ambiguous (overlapping logic from two different tasks, unclear which side should win), the orchestrator dispatches an ad-hoc `general-purpose` subagent scoped to just that one merge — hand it the two branch tips, the conflicting files, and both tasks' issue descriptions for context — rather than resolving it blind or stalling the whole batch on it. The same escalation applies to a task `mergepush` reports as failed (a push rejection, or a merge failure) — it stops on that one task and reports the error rather than guessing at a fix.
...
5. **Pushing a task's branch and opening its PR** — this step is `mergepush`'s job, not the orchestrator's own; `implement`/`validate` dispatch it once per batch of finished worker/validator results (§ Division of responsibility above) and wait for it to return before continuing. Step 3 above already chains each task's branch onto its own real `Depends on:` dependency (or `main`), which can fork into a *tree* whenever two tasks depend on the same third one — that's fine here, since each branch just gets its own PR based on whatever it was created from; nothing needs flattening into one stack. `mergepush` pushes the branch straight from its own worktree (`git -C <worktree-path> push origin <branch-name>` — no checkout collision, since nothing here ever needs that branch checked out a second time anywhere), then opens (or, on a re-run, finds) that branch's PR with `gh pr create --head <branch-name> --base <parent>` / `gh pr view <branch-name>`. Full mechanics, including the exact commands and failure handling: `tools/project-manager/agents/mergepush.md`.

   Removing the original task's worktree (`git worktree remove .claude/worktrees/<task-issue-number>`) happens last and best-effort — it's cleanup, not a prerequisite for any of the above.

   **PR content.** The first time `mergepush` creates a task's PR (`gh pr view <branch> --json number,url` failed beforehand, telling it apart from a re-run against an already-open PR), it sets title/body in the same `gh pr create` call:
...
6. **Continuous merge to trunk.** Right after pushing/opening PRs for a batch (step 5), attempt to land onto `main` every task that's ready: `Done` (validated per § Worker lifecycle), with every task it depends on either already merged or itself `Done` in this same batch. The orchestrator is what knows each task's swimlane `Status` (`gh project item-list`), so it tells `mergepush` which branches currently qualify; `mergepush` merges each qualifying task's PR individually, **in dependency order**, since a dependent's PR base may still point at its dependency's branch until that dependency actually lands.

   Before merging a task, `mergepush` checks it's actually safe to merge right now — `Done` on the Project board only means validation passed locally, not that CI or GitHub's own mergeability check has: `gh pr view <branch-name> --json mergeable,mergeStateStatus,statusCheckRollup`. A check still `PENDING`/`IN_PROGRESS` means wait for a later batch. `mergeable` not `MERGEABLE`, or any check `FAILURE`/`ERROR`/`CANCELLED`, means this task isn't ready — skip it and its dependents in this batch, same as a merge failure below. Only once CI has finished clean does it merge, one call per task, with no `--auto` (nothing left to wait on, and `--auto` would otherwise queue an unattended merge that lands later against a since-changed check result):
   ```sh
   gh pr merge <branch-name> --squash
   ```
   Unlike a single stacked-PR merge, there's no "merge the top and everything below it" shortcut, but there's also no shared stack state to keep consistent. Pick a merge method with `--squash`, `--rebase`, or `--merge` to match whatever the plan needs.

   A task that's still mid-flight (`Scaffold`/`Implementation`/`Testing`) or blocked on an undone dependency is simply left for a later batch — not a failure. A task whose CI hasn't finished, has failed, or isn't `MERGEABLE`, or a merge that fails outright (a stale approval, a race since the check above), is likewise a non-error condition: leave that task (and its dependents, for this batch) unmerged and retry on the next batch. Never fall back to merging on a failed or still-running CI run — that's what breaks `main`.
...
| mergepush | `haiku` | Bounded, mechanical push/PR integration — no reasoning about code, just `git`/`gh` plumbing on commits already made |
