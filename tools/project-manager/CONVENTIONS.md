...
6. **After a subagent finishes** — this step is `mergepush`'s job, not the orchestrator's own; `implement`/`validate` dispatch it once per batch of finished worker/validator results (CONVENTIONS.md § Division of responsibility above) and wait for it to return before continuing:
   ```sh
   ...
   git checkout --detach HEAD
   ```
   `submit` is idempotent — safe to run after every phase, not just once. It pushes and opens (or refreshes) the task's draft PR. This is the pipeline's sole PR-creation path — no persona ever opens a PR directly with `gh pr create`.
