# App Registry — Delivery Plan: Phase History
...
**The AR-5b+ cutover itself has not happened** — no domain is at adoption
stage `allocate`. Its scope, exit criteria, and the decided semver-semantics
addendum are forward-looking, not historical, so they live in
[PLAN.md](PLAN.md) → "AR-5 — remaining work" instead of here.
...
landed after AR-7f — see PLAN.md's "Current status" section.
