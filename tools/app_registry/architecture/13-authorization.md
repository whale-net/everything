...
| `app-registry-builder` | `ReleaseRegistry.NotifyBuildComplete` | Same builder credential CI already holds -- the build job reports its own terminal state onto the release run's Temporal workflow (the notify half of the notify-vs-poll split, see `architecture/08-release-lifecycle/04-run-log.md`); see `server/handlers/release.go`'s `NotifyBuildComplete` doc comment |
| `app-registry-builder` | `ReleaseRegistry.ReportTargetProgress` | Same builder credential -- `release_helper_go` reports one target's intra-build progress (building/built/pushed), ahead of `NotifyBuildComplete`'s batch-wide terminal signal; `building` is accepted for any target kind, `built`/`pushed` are image-only; see `server/handlers/release.go`'s `ReportTargetProgress` doc comment |
| `app-registry-admin` | `EnvironmentRegistry` (writes), `SetAppStatus`, `AdoptArtifact` (AR-7e) | Human only |
| *(public / anonymous)* | All read RPCs (`GetApp`, `ListApps`, `ListCharts`, `GetArtifact`, `ListArtifacts`, `ResolveArtifact`, `ListArtifactPins`, `CheckChartHermeticity`, `GetEnvironmentState`, `ListPromotions`, `ListPromotionEvents`, `GetPromotionDetails`, `GetEnvironment`, `ListEnvironments`, `GetReleaseRun`, `ListBuilds`, `GetBuild`, `ListReconcileRuns`, `GetRelease`, `ListReleases`, `ListReleaseAttempts`) | None (anonymous access permitted) |
