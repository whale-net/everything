...
| `app-registry-builder` | `ReleaseRegistry.ReportTargetProgress` | Same builder credential -- `release_helper_go` reports one image target's intra-build progress (built/pushed), ahead of `NotifyBuildComplete`'s batch-wide terminal signal; see `server/handlers/release.go`'s `ReportTargetProgress` doc comment |
