...
| Enforce "charts may only pin registry-known images" at chart **compose** time, repo-wide, from day one | Originally rejected in favor of a per-domain gate (built as AR-7f): repo-wide, no chart could build until every member app had released through the registry once. That per-domain gate is gone now — chart hermeticity is unconditional and repo-wide (see `10-compose-time-hermeticity.md`) — because by the time it was enforced, every domain was already publishing through the registry, so the original rejection reason no longer applied. |
