...
`PublisherInterface.PublishReleaseRun(releaseRunID, eventKind, eventStatus)` and `Publisher.PublishReleaseRun` (`events/publisher.go`) exist as of #1700, wired at both publish points as of #1702. `eventKind` is `"release_target_" + state` (`release_target_queued`, `release_target_building`, `release_target_built`, `release_target_pushed`, `release_target_publishing`, `release_target_recording`, `release_target_succeeded`, `release_target_failed`); `eventStatus` is `"pending"` for every non-terminal state and `"succeeded"`/`"failed"` for the two terminal ones. `building`/`built`/`pushed` are reported mid-build by `release_helper_go` via the builder-authenticated `ReportTargetProgress` RPC, ahead of the batch-wide `NotifyBuildComplete` terminal signal, and are best-effort: a target that never reports one is simply never observed in that state, and reaches it anyway because `RecordTargetState` walks forward from whatever state it is already in. `building` is reported for any kind (image or chart) and is the only thing that moves a target into `building` -- the workflow does not do it at dispatch, so a multi-target run shows only the target actually being built. `built`/`pushed` remain image-only. Both fields are **advisory only**, same convention as promotion events (see `21-promotion-sse.md`'s Post-commit best-effort publish section): the `/releases/{id}` SSE route re-reads `release_run`/`release_run_target` state at delivery time rather than rendering this payload, and never treats the target identity carried implicitly by a publish as something to act on directly.
