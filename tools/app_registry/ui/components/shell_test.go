...
func TestShell_RendersEveryNavItemExactlyOnce(t *testing.T) {
	...
	navEnd := strings.Index(body[navStart:], "</ul>")
	if navEnd == -1 {
		t.Fatalf("nav <ul> never closes in %q", body)
	}
	navSection := body[navStart : navStart+navEnd+len("</ul>")]
	// Exactly seven <li> elements within the nav <ul> -- guards against an
	// extra hardcoded item sitting alongside the real nav list.
	if got := strings.Count(navSection, "<li>"); got != len(wantNavItems) {
		t.Errorf("expected exactly %d <li> nav items in the nav <ul>, got %d in %q", len(wantNavItems), got, navSection)
	}
}
