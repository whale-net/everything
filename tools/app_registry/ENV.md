...
| `RELEASE_WORKSPACE_ROOT` | *(unset)* — optional | Working directory `ResolvePlan` runs `RELEASE_PLAN_BINARY_PATH plan` from, and `FinalizePublish` runs `finalize-app`/`finalize-chart` from. **No longer required to be a full monorepo checkout.** `ResolvePlan` resolves each target's Domain/Name/AppType via the registry itself and passes it as `--apps-metadata`/`--charts-metadata` (bazel-free — see `worker/release/plan.go`'s package doc comment); when unset, it uses a fresh scratch temp directory per invocation. `FinalizePublish`'s `finalize-app`/`finalize-chart` still need `helm`/`git` on `PATH` (bundled into the worker's image the same way, via `//tools/helm_cli:helm_cli_tar`; `git` already comes from the `@git_base` base image) and, for `--create-git-tag`, a real authenticated git remote — when unset, `FinalizePublish` performs its own shallow, authenticated clone per invocation (`cloneWorkspace`, reusing `RELEASE_GITHUB_APP_*`/`RELEASE_GITHUB_REPO_*`'s credentials) rather than requiring a pre-provisioned checkout. Setting this var still overrides both with a fixed directory (e.g. for local debugging). |
