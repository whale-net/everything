...
| `vars.APP_REGISTRY_BUILDER_ENV` | Repository variable | `GRPC_AUTH_CLIENT_ID=app-registry-builder-<value>`, falls back to `dev` when unset | `release.yml` recording steps, (AR-7f) "Build helm charts with versioning", `release-v2.yml`'s notify-temporal job (`NotifyBuildComplete`) and build-release-artifacts job (`ReportTargetProgress`) |
