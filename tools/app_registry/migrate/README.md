...
| `022_drop_domain_adoption` (AR-5 cutover) | Drops `domain_adoption`. The AR-5 cutover makes every domain unconditionally "allocate" — `AllocateVersion`, `CheckChartHermeticity`, and `RecordArtifact`'s pre-AR-7 direct-create fallback no longer branch on a per-domain stage, so the table nothing reads anymore is dropped. |
...
`domain_adoption` existed from migration `001` through migration `021` —
one row per domain, with a stage of `observe` / `promote` / `allocate`,
gating the per-domain cutover described in
[ARCHITECTURE.md "Resolved questions"](../architecture/19-resolved-questions.md). The AR-5
cutover made every domain unconditionally "allocate", so the table had
nothing left to gate; migration `022` drops it.
