...
| `022_drop_domain_adoption` | Drops `domain_adoption` — `AllocateVersion`, `CheckChartHermeticity`, and `RecordArtifact` no longer branch on a per-domain stage; every domain is unconditionally allocated. |
...
the table had nothing left to gate; migration `022` drops it.
