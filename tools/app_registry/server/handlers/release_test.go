...
func progressReq(releaseRunID string, githubRunID int64, owner string, state pb.ReleaseRunTargetState) *pb.ReportTargetProgressRequest {
	return &pb.ReportTargetProgressRequest{
		ReleaseRunId:  releaseRunID,
		GithubRunId:   githubRunID,
		OwnerFullName: owner,
...
// TestReportTargetProgress_Authorization mirrors
// TestNotifyBuildComplete_Authorization -- same builder role, no new role.
func TestReportTargetProgress_Authorization(t *testing.T) {
	srv, _, _, runID := notifyFixture(t)

	t.Run("correct role allowed", func(t *testing.T) {
		srv2, _, _, runID2 := notifyFixture(t)
		resp, err := srv2.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID2, 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
		if err != nil {
			t.Fatalf("expected builder to be allowed, got %v", err)
		}
		if !resp.Signaled {
	...
	t.Run("wrong role is PermissionDenied", func(t *testing.T) {
		_, err := srv.ReportTargetProgress(ctxWithRoles(auth.RolePromoterDev), progressReq(runID, 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
		requireCode(t, err, codes.PermissionDenied, "ReportTargetProgress")
	})
}

// TestReportTargetProgress_Validation pins the request validation order:
// missing release_run_id/owner_full_name are InvalidArgument, an
// unsupported state is InvalidArgument, and the nil-temporal guard is
// FailedPrecondition.
func TestReportTargetProgress_Validation(t *testing.T) {
	srv, _, _, runID := notifyFixture(t)

	_, err := srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq("", 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
	requireCode(t, err, codes.InvalidArgument, "ReportTargetProgress (missing release_run_id)")

	_, err = srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 42, "", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
	requireCode(t, err, codes.InvalidArgument, "ReportTargetProgress (missing owner_full_name)")

	_, err = srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_SUCCEEDED))
	requireCode(t, err, codes.InvalidArgument, "ReportTargetProgress (unsupported state)")

	srvNoTemporal, _ := newReleaseFixture()
	_, err = srvNoTemporal.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
	requireCode(t, err, codes.FailedPrecondition, "ReportTargetProgress (nil temporal client)")
}

// TestReportTargetProgress_SignalsWorkflow pins the success path: the
// handler resolves the run and signals its TemporalWorkflowID with
// release.SignalTargetProgress carrying release.TargetProgressSignal.
func TestReportTargetProgress_SignalsWorkflow(t *testing.T) {
	srv, repo, temporal, runID := notifyFixture(t)
	if err := repo.ReleaseRuns().SetBuildRef(context.Background(), runID, "410", "https://run/410"); err != nil {
		t.Fatalf("SetBuildRef: %v", err)
	}

	resp, err := srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 410, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
	if err != nil {
	...
	sig := temporal.signals[0]
	if sig.WorkflowID != "release-workflow-1" || sig.RunID != "" || sig.SignalName != release.SignalTargetProgress {
		t.Fatalf("unexpected signal identity: %+v", sig)
	}
	payload, ok := sig.Arg.(release.TargetProgressSignal)
	if !ok {
		t.Fatalf("expected TargetProgressSignal payload, got %T", sig.Arg)
	}
	if payload.GitHubRunID != "410" || payload.OwnerFullName != "demo-svc" || payload.Kind != repository.ArtifactKindImage || payload.State != repository.ReleaseRunTargetStateBuilt {
		t.Fatalf("unexpected payload: %+v", payload)
	}
}
...
func TestReportTargetProgress_RunIDMismatch(t *testing.T) {
	...
	_, err := srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 411, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_BUILT))
	requireCode(t, err, codes.FailedPrecondition, "ReportTargetProgress (run id mismatch)")
	if len(temporal.signals) != 0 {
		t.Fatalf("a mismatched report must not reach SignalWorkflow, got %d calls", len(temporal.signals))
	}
}

// TestReportTargetProgress_WorkflowNotFound mirrors
// TestNotifyBuildComplete_WorkflowNotFound's informational no-op.
func TestReportTargetProgress_WorkflowNotFound(t *testing.T) {
	srv, _, temporal, runID := notifyFixture(t)
	temporal.err = serviceerror.NewNotFound("workflow execution not found")

	resp, err := srv.ReportTargetProgress(ctxWithRoles(auth.RoleBuilder), progressReq(runID, 42, "demo-svc", pb.ReleaseRunTargetState_RELEASE_RUN_TARGET_STATE_PUSHED))
	if err != nil {
		t.Fatalf("expected NotFound to be swallowed as Signaled=false, got %v", err)
	}
	if resp.Signaled {
