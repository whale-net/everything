...
| `KRILL_MCP_WHAGENT_JWKS_URL` | — (defaults in `krill/Tiltfile` for local dev) | whagent-net's own JWKS endpoint. Both this and `KRILL_MCP_WHAGENT_ISSUER` must be set to enable the agent front door (`server.WhagentAuthConfig`) -- left unset, `mcp` mounts only the mcpauth door, mirroring `audience_score_system/mcp`'s own pre-FR12(a) fallback. `krill/Tiltfile` defaults this to whagent-net's in-cluster `whagent-net-api` Service address (namespace `whagent-net-local-dev`), overridable via `BUILD_WHAGENT_ENV`/`KRILL_MCP_WHAGENT_JWKS_URL`, exactly mirroring `audience_score_system/Tiltfile`'s own `ASS_WHAGENT_JWKS_URL` block -- the JWKS endpoint only actually resolves if `whagent_net`'s own `tilt up` is running alongside this one. Enabling the door here is not sufficient on its own for a real whagent-net-authenticated caller to reach krill: whagent-net has no `agent_definition` row registered for krill yet (`whagent_net/config/agents.yaml`'s doc comment -- "a human inserts these rows by hand"), so no whagent-net session can mint a Claim audienced at this `mcp` instance until one is added. |
| `KRILL_MCP_WHAGENT_ISSUER` | — (defaults in `krill/Tiltfile` for local dev) | whagent-net's own issuer identifier, verified against every whagent Claim `mcp` accepts. Same default/override mechanics as `KRILL_MCP_WHAGENT_JWKS_URL` above. |
