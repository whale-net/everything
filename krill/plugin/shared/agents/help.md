...
| Task manifest exists (Milestone path), tasks unclaimed or in progress | `krill-work:implement <milestone-id>` (needs the manifest `plan` returned) | Krill-native — `worker`/`validator`'s `claim_task`/`complete_task` work from this dispatch (the #2930 persona gate is fixed). A `forbidden` here is a real regression, not a known condition. |
...
| Noticed something out of scope while doing something else (Milestone path) | *(no skill — call `record_note {task_id, kind: "scope-note", body}` directly)* | Works from this dispatch (the #2930 persona gate is fixed). A `forbidden` is a real regression — report it, don't fall back to GitHub. |
