...
| Design session signed off, no GitHub tracking issue/Project board yet | `krill-work:plan <feature-set-id>` | **TODO(M3)**: today this still creates a GitHub Project + task issues citing the FeatureSet id in the body (no krill `PointerArtifact` MCP write path exists yet — see CONVENTIONS.md). |
| Project board exists, tasks unclaimed or in progress | `krill-work:implement <issue-number>` | Idempotent — safe to re-run. **TODO(M4)**: will become MCP `claim`/`heartbeat` calls once krill's work-tracking surface ships. |
| All task issues `Done`, haven't checked the whole system yet | `krill-work:validate <issue-number>` | Runs system-validator against Tilt; findings route back to planner as new tasks. |
| Validation found bugs / findings issues exist | `krill-work:implement <issue-number>` again | Findings land on the board like any other task. |
| Signed-off design, and requester wants the whole thing driven to a merged stack without re-running each phase by hand | `krill-work:loop-plan-implement-validate <issue-number>` | Chains `plan`→`implement`→`validate`, re-looping on findings, until the stack is merged. |
| "Where does this plan/product stand right now?" / unsure what phase something is in | `status <design-session-id\|issue-number>` | Pure read — always safe to run first. Shared skill, works from either plugin. |
| Noticed something out of scope while doing something else | *(no skill — file a scope note directly, per project-manager's CONVENTIONS.md § Scope notes)* | krill has no native scope-note entity yet; this still rides on GitHub until one exists. |
