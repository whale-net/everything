...
| [`ARCHITECTURE/36-resolve-deferred-non-goal.md`](ARCHITECTURE/36-resolve-deferred-non-goal.md) | Resolve — the third LB3 shape (FR d0021a0f, FR 19123858) — promote (close + successor, re-kinded) vs retire (close without successor, reusing void), the `deferred`-only check, and why promote needs its own register rather than a row in `void_event` |