...
// its own pre-filtered endpoint, distinct from the work-axis surface
// (workMountPath, below). Follows whagent_net's `/mcp/readonly` vs
// `/mcp/ops` split (ARCHITECTURE.md "Domain-owned MCP servers and the
// tool contract"): mounting under a granularity-named path now is cheap,
// and folding every granularity onto bare "/" would make a later mount
// awkward to add without breaking this one's clients.
...
// "on the whole binary."
//
// The design-session, milestone, and delivery-axis tools live here.
// create_task's M4 siblings (claim_task/heartbeat_task/complete_task/
// record_note/transition_note_lifecycle/declare_task_dependencies) do
// NOT -- they moved to workMountPath so the krill-work plugin (worker/
// validator/planner personas) is never handed the design-authoring
// surface just to reach its own task-lifecycle verbs. get_task and
// abandon_task are the deliberate exception: both are also registered on
// workMountPath, but they stay here too so a design-plugin persona
// retains ad hoc read (get_task) and cleanup (abandon_task) access for
// one-off task inspection/unwinding, even though no design persona's own
// instructions call either today.
const designMountPath = "/mcp/design"

// workMountPath is where krill's work-axis task-lifecycle MCP surface is
// mounted -- its own pre-filtered endpoint, alongside specMountPath and
// designMountPath, never folded onto either. create_task,
// declare_task_dependencies, claim_task, heartbeat_task, complete_task,
// record_note, and transition_note_lifecycle live ONLY here: the
// krill-work plugin points at this mount (plus specMountPath) instead of
// designMountPath, so worker/validator/planner personas can drive a
// task's lifecycle without also gaining the design-session/milestone/
// delivery-axis write surface designMountPath carries. get_task and
// abandon_task are registered here AND on designMountPath (see that
// const's doc comment) -- the one intentional exception to "a tool lives
// on exactly one mount."
const workMountPath = "/mcp/work"

// opsMountPath is where krill's operator surface (M5, issue #2867) is
// mounted -- its own pre-filtered endpoint, alongside specMountPath,
// designMountPath, and workMountPath, never folded onto any of them. FR6-FR9 of the M5 root plan
...
func NewDualAuthHTTPHandler(specSrv, designSrv, workSrv, opsSrv *mcp.Server, credentials auth.CredentialStore, whagentCfg WhagentAuthConfig, resourceMeta ResourceMetadataConfig) http.Handler {
	...
	workGuarded := DualAuthHTTPHandler(mcpHandlerFor(workSrv), credentials, whagentCfg, opts)
	opsGuarded := DualAuthHTTPHandler(mcpHandlerFor(opsSrv), credentials, whagentCfg, opts)

	return newMux(specGuarded, designGuarded, workGuarded, opsGuarded, resourceMeta)
}
...
// specGuarded, the one at designMountPath guarded by designGuarded, the
// one at workMountPath guarded by workGuarded, and the one at opsMountPath
// guarded by opsGuarded -- shared by NewHTTPHandler and
