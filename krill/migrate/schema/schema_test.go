...
// expects. See whagent_net/migrate/schema/schema_test.go and
// audience_score_system/migrate/schema/schema_test.go for the precedent
// this mirrors.
func TestMigrationsEmbedResolves(t *testing.T) {
	...
	if len(entries) == 0 {
		t.Fatalf("Migrations.ReadDir(%q) returned no entries -- expected at least 001_scope.up/down.sql", Dir)
	}
