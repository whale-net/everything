...
(#2237, FR10/FR13).** `SessionService`'s two read RPCs (`GetSession`,
`ReadTranscript`) apply no ownership check at all — any authenticated
caller may read any session, whoever started or controls it (FR2/C14:
on-call viewers are the point). The two write RPCs that act on a running
session (`SendTurn`, `StopSession`) are gated by `canControl`, which grants
control through either of two independent rules:

- **(a) the on-behalf-of subject.** The caller may control a session when it
  *is* (or is acting for) the session's `on_behalf_of` `(iss, sub)` pair
  (FR1/C13), matched on both fields per LB2 (same `sub` under a different
  `iss` does not control). `on_behalf_of == subject` for every non-delegated
  row, so this rule is behavior-preserving on existing data.
- **(b) the allowlisted starting caller.** The caller is the session's
  `subject` — the identity that actually started it — *and* the current
  call's own Keycloak `client_id` is on the same allowlist
  (`WHAGENT_ON_BEHALF_OF_ALLOWED_CLIENT_IDS`) that gates the delegated
  `on_behalf_of` start. This lets the client that started a delegated
  session keep driving it after handing it off, without widening control to
  arbitrary callers. The `client_id` is read fresh from the current call's
  `grpcauth.Claims.ClientID` — never persisted on the session row, never
  derived from `sess.Subject`, never cached — so removing a client from the
  allowlist takes effect on the very next `SendTurn`/`StopSession`. The
  allowlist is fail-closed (NFR1): empty/unset means branch (b) never
  grants.

There is no admin override in M1 — a caller that is neither the acting nor
the on-behalf-of subject, nor an allowlisted starter, simply cannot control
a session it didn't start. `whagent_net/api/handlers/session.go`'s
`canControl` is the single place this control rule lives; no other handler
re-derives it.
