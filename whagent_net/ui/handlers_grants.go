...
func buildGrantRows(ctx context.Context, index grantIndexLister, store grpcauth.Store, iss, sub string, logger *slog.Logger) ([]pages.GrantRow, error) {
	...
	for _, e := range entries {
		...
		if err != nil {
			// A malformed domain recorded in the index would mean some
			// earlier consent flow let an invalid domain through --
		...
		case errors.Is(statusErr, grpcauth.ErrGrantNotFound):
			...
			logger.Warn("grant recorded in index has no matching store row", "domain", e.Domain)
		case statusErr != nil:
...
// grantkey.ForDomain(domain) validates/derives the grant key, then
...
func revokeGrant(ctx context.Context, store grpcauth.Store, iss, sub, domain string, logger *slog.Logger) error {
	...
	// grant, which domain. This is a self-service revoke, so the revoker
	...
	// logs token material.
	logger.Info("delegated grant revoked", "revoked_by_sub", sub, "subject_iss", iss, "subject_sub", sub, "domain", domain)
	return nil
}
...
func (app *App) handleGrantsRevoke(w http.ResponseWriter, r *http.Request) {
	...
	if err := revokeGrant(ctx, app.grant.Store, app.oidcIssuer, user.Sub, domain, logger); err != nil {
		switch {
		case errors.Is(err, grpcauth.ErrGrantNotFound):
			// Either a malformed domain (grantkey.ForDomain rejected it)
			// or no persisted row for (this operator, this domain) --
			// including the case a tampered request named a domain/
