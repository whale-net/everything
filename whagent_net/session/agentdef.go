...
const agentDefinitionColumns = `id, agent_id, domain, version, model, model_definition_id, tool_set, max_turns, max_cost_usd, required_role, created_at`

func scanAgentDefinition(row pgx.Row) (*AgentDefinition, error) {
	var def AgentDefinition
	var toolSet json.RawMessage
	if err := row.Scan(
		&def.ID, &def.AgentID, &def.Domain, &def.Version, &def.Model, &def.ModelDefinitionID, &toolSet,
...
func (s agentDefinitionStore) Upsert(ctx context.Context, def *AgentDefinition) error {
	...
	`, def.AgentID, def.Domain, def.Version, def.Model, def.ModelDefinitionID, toolSet, def.MaxTurns, def.MaxCostUSD, def.RequiredRole).Scan(&def.ID, &def.CreatedAt)
	if err != nil {
