...
type TranscriptStore interface {
	...
	// is what lets CallModel's activity input carry only a reference
	// (SessionID+Turn, both already required fields) instead of the full
	// tool list on every one of a turn's model calls (ARCHITECTURE.md
	// "Activity payload discipline"). Safe to call more than once for the
	// same (SessionID, Turn) -- a retried ListToolDefinitions activity
	// overwrites with the same deterministically-refetched list.
	...
	// CallModel treats as "no tools attached this turn" rather than a
	// fault.
