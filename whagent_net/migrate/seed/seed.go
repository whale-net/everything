...
//     WithSeeder shape);
//  2. compares model/tool_set/max_turns/max_cost_usd/required_role against
...
// every config entry's model against llm.Catalog.Supports *before*
// writing any row, so a bad model fails the whole seeder run loudly
// rather than writing a half-seeded table -- the check-all-then-write-all
// two-pass shape below is what makes that guarantee hold even for a
// config with more than one entry. whagent_net/config.Load already
// enforces the pure shape rules (missing agent_id/model, empty tool_set)
...
// (agent_id, version) pair -- see this package's doc comment).
func seedOne(ctx context.Context, db *sql.DB, a config.AgentDefinitionConfig) error {
	...
	`, a.AgentID, nextVersion, a.Model, toolSetJSON, a.MaxTurns, a.MaxCostUSD, role); err != nil {
...
func latestAgentDefinition(ctx context.Context, db *sql.DB, agentID string) (latestRow, bool, error) {
	...
	err := db.QueryRowContext(ctx, `
		SELECT version, model, tool_set, max_turns, max_cost_usd, required_role
		FROM agent_definition
		WHERE agent_id = $1
		ORDER BY version DESC
		LIMIT 1
	`, agentID).Scan(&row.Version, &row.Model, &toolSetJSON, &row.MaxTurns, &row.MaxCostUSD, &row.RequiredRole)
	if err != nil {
