...
func (a *Activities) CallModel(ctx context.Context, in CallModelInput) (CallModelResult, error) {
	...
	encoded, ok, err := a.Store.Transcript().ReadTurnToolDefs(ctx, in.SessionID, in.Turn)
	if err != nil {
		return CallModelResult{}, fmt.Errorf("call model: read turn tool defs: %w", err)
	}
	if ok {
		if err := json.Unmarshal(encoded, &tools); err != nil {
			return CallModelResult{}, fmt.Errorf("call model: decode turn tool defs: %w", err)
		}
	}

	resp, err := a.LLM.Complete(ctx, llm.Request{Model: in.Model, Messages: messages, Tools: tools, Provider: in.Provider})
	if err != nil {
...
// into CallModelInput.Tools on every model call a turn's tool loop made,
// duplicating the full tool catalog (JSON schemas included) into Temporal
// workflow history up to max_tool_iterations+1 times per turn. This
// activity instead persists that list to a `turn_tool_defs` row
// (SaveTurnToolDefs) keyed on (in.SessionID, in.Turn), an activity-internal
// write invisible to workflow history, and CallModel reads it back the same
// way (ARCHITECTURE.md "Activity payload discipline"). Returning the full
// list here would still only cost workflow history one copy per turn (this
// activity already runs once per turn, not once per tool-loop iteration);
// the actual bloat processTurn's tool loop was causing came from forwarding
// that result into CallModelInput.Tools on every one of that turn's model
// calls (up to max_tool_iterations+1) -- moving the data off the workflow
// boundary entirely, rather than merely deduplicating it, is what fixes
// that at the source.
