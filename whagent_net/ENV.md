...
| `WHAGENT_GRANT_ADMIN_ROLE` | ui | — | The Keycloak realm role name that gates FR14/FR15's admin all-operators grant list and revoke page (`GET /admin/grants`, `POST /admin/grants/revoke`, issue #2433). Checked fresh on every request against the roles carried by the signed-in operator's *current* access token (`htmxauth.DBSessionManager.GetAccessToken`, which refreshes against Keycloak — NFR3), never the 24h-cached `htmxauth.GetUser(ctx).Roles` snapshot, so a revoked admin role stops working on the very next request rather than up to a day later. Left unset, the admin page 403s for every operator — there is no "everyone is admin" default. Realm-side creation and assignment of this role to specific operators is deployment/runbook work, not covered by this plan (#2421). |
...
| `WHAGENT_ON_BEHALF_OF_ALLOWED_CLIENT_IDS` | api | (unset) | Comma-separated Keycloak `client_id`s allowed to set `StartSessionRequest.on_behalf_of` and create a delegated session (FR9/FR11/FR12). Whitespace around each entry is trimmed and empty entries are ignored. The gate is on the *caller's own* `client_id` (the token's `azp`/`client_id`), so a human caller can never delegate just by being authenticated. Unset or empty fails closed: any request carrying `on_behalf_of` is `PermissionDenied` (NFR1). Requests that leave `on_behalf_of` unset are unaffected -- they record `on_behalf_of_*` identical to `subject_*`. Production sets this to fcm's whagent-net service-account `client_id` (fcm's `WHAGENT_CLIENT_ID`) as its only entry (FR12); the local Tilt config leaves it empty since it runs `GRPC_AUTH_MODE=none` with no Keycloak realm. |
...
| `WHAGENT_UI_ASS_LINK_URL` | ui | — | ASS `web`'s own externally-reachable base URL — the redirect target `handlers_link.go`'s `handleLinkASSStart` (FR1/FR2, issue #2596) carries a freshly minted `WHAGENT_UI_SIGNING_KEY`-signed assertion to (`{WHAGENT_UI_ASS_LINK_URL}/link/whagent?token=...`). Optional, non-fatal-degrade like `WHAGENT_GRANT_*`/`WHAGENT_S3_BUCKET`: left unset, `/grants` never renders the "Link ASS identity" action and `POST /link/ass` answers a plain "not configured" response — never a 500, never a redirect to an empty host. `whagent_net/Tiltfile` defaults this to `http://localhost:8083` (ASS `web`'s own browser-facing port-forward, only reachable when `audience_score_system`'s Tiltfile is also up), overridable via `BUILD_WHAGENT_ENV`/`WHAGENT_UI_ASS_LINK_URL` in a local `.env`, same cross-Tiltfile pattern as `ASS_WHAGENT_UI_JWKS_URL`/`ASS_WHAGENT_UI_ISSUER` above. |
