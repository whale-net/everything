...
// mcpdomain_integration_test.go's job). Each fake method panics if called
...
// TestDomainForAgent_ReturnsDomainFromLatestDefinition proves the happy
// path: DomainForAgent forwards to GetLatest and returns its Domain
// unchanged.
func TestDomainForAgent_ReturnsDomainFromLatestDefinition(t *testing.T) {
	store := &fakeAgentDefinitionStore{
		getLatestFunc: func(_ context.Context, agentID string) (*session.AgentDefinition, error) {
			...
			return &session.AgentDefinition{AgentID: agentID, Domain: "audience_score_system", Version: 3}, nil
		},
	}
	r := New(store)

	domain, err := r.DomainForAgent(context.Background(), "research-agent")
	require.NoError(t, err)
	assert.Equal(t, "audience_score_system", domain)
}

// TestDomainForAgent_UnknownAgent_ReturnsErrNotFound proves GetLatest's
// documented "no rows -> nil, nil" contract maps to a distinguishable
// ErrNotFound, not an empty string with a nil error.
func TestDomainForAgent_UnknownAgent_ReturnsErrNotFound(t *testing.T) {
	...
	domain, err := r.DomainForAgent(context.Background(), "does-not-exist")
	assert.Empty(t, domain)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrNotFound), "unknown agent id must be a distinguishable ErrNotFound, got: %v", err)
}

// TestDomainForAgent_TransportError_NotMistakenForErrNotFound proves a
...
func TestDomainForAgent_TransportError_NotMistakenForErrNotFound(t *testing.T) {
	store := &fakeAgentDefinitionStore{
...
// TestDomainForSession_MalformedSessionID_ReturnsErrNotFoundWithoutCallingStore
...
func TestDomainForSession_MalformedSessionID_ReturnsErrNotFoundWithoutCallingStore(t *testing.T) {
	store := &fakeAgentDefinitionStore{}
	r := New(store)

	domain, err := r.DomainForSession(context.Background(), "not-a-uuid")
	assert.Empty(t, domain)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrNotFound))
}

// TestDomainForSession_UnknownSession_ReturnsErrNotFound proves
...
func TestDomainForSession_UnknownSession_ReturnsErrNotFound(t *testing.T) {
	...
	domain, err := r.DomainForSession(context.Background(), uuid.NewString())
	assert.Empty(t, domain)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrNotFound))
}

// TestDomainForSession_CurrentAssignmentTransportError_NotMistakenForErrNotFound
// mirrors the DomainForAgent transport-error case for the
// CurrentAssignment call.
func TestDomainForSession_CurrentAssignmentTransportError_NotMistakenForErrNotFound(t *testing.T) {
	store := &fakeAgentDefinitionStore{
...
// TestDomainForSession_AssignedVersionMissing_ReturnsErrNotFound proves
...
func TestDomainForSession_AssignedVersionMissing_ReturnsErrNotFound(t *testing.T) {
	sid := uuid.New()
	store := &fakeAgentDefinitionStore{
	...
	domain, err := r.DomainForSession(context.Background(), sid.String())
	assert.Empty(t, domain)
	require.Error(t, err)
	assert.True(t, errors.Is(err, ErrNotFound))
}

// TestDomainForSession_GetVersionTransportError_NotMistakenForErrNotFound
// mirrors the transport-error case for the GetVersion call.
func TestDomainForSession_GetVersionTransportError_NotMistakenForErrNotFound(t *testing.T) {
	sid := uuid.New()
	store := &fakeAgentDefinitionStore{
...
// TestDomainForSession_UsesAssignedVersionNotLatest proves the doc
// comment's core promise -- "never the current/latest version" -- at the
// unit level: GetVersion is called with the exact version
// CurrentAssignment returned, and its Domain (not some other version's)
...
func TestDomainForSession_UsesAssignedVersionNotLatest(t *testing.T) {
	sid := uuid.New()
	store := &fakeAgentDefinitionStore{
		currentAssignmentFunc: func(context.Context, uuid.UUID) (*session.SessionAgent, error) {
			return &session.SessionAgent{SessionID: sid, AgentID: "research-agent", AgentVersion: 1}, nil
		...
		getVersionFunc: func(_ context.Context, agentID string, version int) (*session.AgentDefinition, error) {
			...
			return &session.AgentDefinition{AgentID: agentID, Version: version, Domain: "old-domain"}, nil
		},
	}
	r := New(store)
	...
	domain, err := r.DomainForSession(context.Background(), sid.String())
	require.NoError(t, err)
	assert.Equal(t, "old-domain", domain)
}
