...
// under, for AuthMiddleware to read back off the request and exchange.
...
// routes the call onto AuthMiddleware's OAuth2/token-exchange branch;
...
// (req.GetExtra().TokenInfo) and places a bearer token on ctx via
// grpcauth.WithUserToken -- the exact mechanism
// grpcauth.NewUserTokenDialOption reads from when a tool (../tools/*.go)
// calls `api` (issue #2120's Implementation section, "Identity
// pass-through").
//
// Two branches, matching NewVerifier's two TokenInfo shapes:
//
//   - identityExtraKey present (the OAuth2 path, FR9/issue #2249): the
//     resolved (iss, sub) is exchanged for a short-lived, real
//     Keycloak-signed JWT via exchanger.Exchange (tokenexchange.go) --
//     never any value read directly off the incoming credential -- and
//     that exchanged JWT is what gets forwarded, exactly as if the
//     operator had presented it themselves. A failed exchange is logged
//     at ERROR by Exchange itself; this middleware rejects the call
//     without retrying or falling back to any other path.
//   - tokenExtraKey present (the manual-token path, unchanged): the raw
//     token is forwarded byte for byte, exactly as before FR9.
//
...
// without one.
func AuthMiddleware(exchanger Exchanger) mcp.Middleware {
	return func(next mcp.MethodHandler) mcp.MethodHandler {
		return func(ctx context.Context, method string, req mcp.Request) (mcp.Result, error) {
			...
			if identity, ok := extra.TokenInfo.Extra[identityExtraKey].(resolvedIdentity); ok {
				jwt, err := exchanger.Exchange(ctx, identity.iss, identity.sub)
				if err != nil {
					return nil, errors.New("unauthenticated: identity exchange failed")
				}
				return next(grpcauth.WithUserToken(ctx, jwt), method, req)
			}
