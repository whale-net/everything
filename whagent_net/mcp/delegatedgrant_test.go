...
// #2426's Testing section item verbatim, exercised at `mcp`'s own
...
// distinguishable from the entirely-unset case (see the sibling test
// below) and from the "no pool at all" case (a separate,
// PG_DATABASE_URL-gated degrade path this function also has).
//
...
// TestInitializeDelegatedGrant_NoPool_DegradesWithoutError proves this
// binary's own PG_DATABASE_URL-gated degrade path (distinct from
// //whagent_net/delegatedgrant.Build's own ErrNotConfigured check): a nil
// pool (PG_DATABASE_URL unset, or the FR9 credential-store connection
// attempt failed) skips delegated-grant construction entirely, even with
// a fully-configured WHAGENT_GRANT_* set -- there is nothing to build a
// Store/Index against.
func TestInitializeDelegatedGrant_NoPool_DegradesWithoutError(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	components, err := initializeDelegatedGrant(context.Background(), fullyConfiguredGrantConfig(), nil, logger)

	if err != nil {
		t.Fatalf("expected no error when pool is nil, got %v", err)
	}
	if components.Source != nil || components.Store != nil || components.Index != nil {
		t.Fatalf("expected a zero-value Components when pool is nil, got %+v", components)
	}
}
