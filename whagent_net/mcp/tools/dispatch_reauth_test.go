...
//
// Red/green (verified by hand, then reverted): every "fails naming domain"
...
// the plain domain-naming wrap was returned instead of a reauthRequiredError)
// and failed exactly on the errors.As(err, *reauthRequiredError)/
// "/mcp/consent?domain=" assertions before being reverted back to green --
...
// TestAcquireGrantTokenNeedsReauth_SessionEntryPoint_FailsNamingDomain_NoRetry
...
// naming the affected domain, and GrantSource.TokenSource is asked for
// exactly once -- no retry (FR18).
func TestAcquireGrantTokenNeedsReauth_SessionEntryPoint_FailsNamingDomain_NoRetry(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "manmanv2", "the failure must name the affected domain")
	assert.Contains(t, err.Error(), "/mcp/consent?domain=manmanv2", "the failure must point at the standalone per-domain consent route (#2428)")
...
// TestAcquireGrantTokenNeedsReauth_AgentEntryPoint_FailsNamingDomain proves
...
func TestAcquireGrantTokenNeedsReauth_AgentEntryPoint_FailsNamingDomain(t *testing.T) {
	identity := mcpidentity.Identity{Iss: "https://keycloak.example.test/realms/whagent", Sub: "operator-reauth-2"}
	ctx := mcpidentity.ContextWithIdentity(context.Background(), identity)

	domainResolver := newFakeDomainResolver()
	domainResolver.agentDomains["agent-1"] = "audience_score_system"
	grant := newFakeGrantSourceNeedsReauth()
	...
	assert.Contains(t, err.Error(), "/mcp/consent?domain=audience_score_system")
	assert.True(t, ctx == outCtx) //nolint:staticcheck

	require.Len(t, grant.calls, 1)
}

// TestNeedsReauth_NoSubstitution_HealthyOtherDomainGrantNeverUsed is the
...
// fails in this test".
func TestNeedsReauth_NoSubstitution_HealthyOtherDomainGrantNeverUsed(t *testing.T) {
	identity := mcpidentity.Identity{Iss: "https://keycloak.example.test/realms/whagent", Sub: "operator-reauth-3"}
	ctx := mcpidentity.ContextWithIdentity(context.Background(), identity)

	domainResolver := newFakeDomainResolver()
	domainResolver.sessionDomains["sess-ass"] = "audience_score_system"
	...
	outCtx, err := resolveGrantTokenForSession(ctx, domainResolver, grant, "sess-ass")
	...
	assert.Equal(t, "audience_score_system", grant.calls[0].grant, "the healthy manmanv2 grant must never be requested as a substitute for the needs_reauth domain actually resolved")
}
...
// TestNeedsReauth_EveryToolEntryPoint_ProducesDomainNamingError is the
// Testing section's "start_session and each existing-session handler each
// produce the same domain-naming error" bullet, run through every
...
// isolation.
func TestNeedsReauth_EveryToolEntryPoint_ProducesDomainNamingError(t *testing.T) {
	const domain = "manmanv2"
	identity := mcpidentity.Identity{Iss: "https://keycloak.example.test/realms/whagent", Sub: "operator-reauth-5"}
	ctx := mcpidentity.ContextWithIdentity(context.Background(), identity)

	newResolver := func() *fakeDomainResolver {
		r := newFakeDomainResolver()
		r.agentDomains["agent-1"] = domain
		r.sessionDomains["sess-1"] = domain
		return r
	}

	cases := []struct {
		name string
		call func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error
	}{
		{"start_session", func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error {
			tool := &startSessionTool{client: fc, domainResolver: dr, grant: gr}
			_, _, err := tool.call(ctx, nil, StartSessionInput{AgentID: "agent-1"})
			return err
		}},
		{"send_turn", func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error {
			tool := &sendTurnTool{client: fc, domainResolver: dr, grant: gr}
			_, _, err := tool.call(ctx, nil, SendTurnInput{SessionID: "sess-1", Input: "x"})
			return err
		}},
		{"stop_session", func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error {
			tool := &stopSessionTool{client: fc, domainResolver: dr, grant: gr}
			_, _, err := tool.call(ctx, nil, StopSessionInput{SessionID: "sess-1"})
			return err
		}},
		{"get_session", func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error {
			tool := &getSessionTool{client: fc, domainResolver: dr, grant: gr}
			_, _, err := tool.call(ctx, nil, GetSessionInput{SessionID: "sess-1"})
			return err
		}},
		{"read_transcript", func(fc *fakeSessionServiceClient, dr DomainResolver, gr GrantSource) error {
			tool := &readTranscriptTool{client: fc, domainResolver: dr, grant: gr}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Contains(t, err.Error(), "/mcp/consent?domain="+domain)
...
// TestNeedsReauth_LogsAtWarningWithDomainField_NeverLogsSecret proves the
...
// table) with the domain in a structured field, and never logs anything
...
func TestNeedsReauth_LogsAtWarningWithDomainField_NeverLogsSecret(t *testing.T) {
	...
	grant := &fakeGrantSource{
		tokenFunc: func(context.Context, string, string) (*oauth2.Token, error) {
			...
			// ever passes domain/subject, never cause itself.
	...
	assert.Equal(t, "manmanv2", record["domain"])
