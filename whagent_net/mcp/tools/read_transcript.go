...
// token via ctx exactly as ../server/auth.go's AuthMiddleware placed it
// there (the manual-token path) or as resolveGrantTokenForSession
// acquired it (the browser-OAuth2 path). Events come back from api
