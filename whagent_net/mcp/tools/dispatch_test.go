...
// dispatch-time domain-resolution + delegated-grant token-acquisition
...
// directly against fakeDomainResolver/fakeGrantSource (never a real
...
// domain" and NFR2 cross-domain-refusal tests below were run against a
// deliberately broken acquireGrantToken (domain name dropped from the
...
// domainResolver or grant at all -- dispatch.go's own doc comment's "nil
...
func TestResolveGrantTokenForAgent_NoIdentityOnContext_IsNoOp(t *testing.T) {
	...
	domainResolver := newFakeDomainResolver()
	grant := &fakeGrantSource{}

	outCtx, err := resolveGrantTokenForAgent(ctx, domainResolver, grant, "agent-1")
	...
	assert.Empty(t, domainResolver.agentCalls, "domainResolver must never be consulted on the manual-token path")
...
func TestResolveGrantTokenForSession_NoIdentityOnContext_IsNoOp(t *testing.T) {
	...
	outCtx, err := resolveGrantTokenForSession(ctx, domainResolver, grant, "sess-1")

	require.NoError(t, err)
	assert.True(t, ctx == outCtx) //nolint:staticcheck
	assert.Empty(t, domainResolver.sessionCalls)
	assert.Empty(t, grant.calls)
}

// TestResolveGrantTokenForAgent_ActiveGrant_ResolvesDomainAndAcquiresExactPair
// is this task's first Testing-section bullet: start_session's dispatch
// sequence resolves the agent's domain, derives the grant key from it
// (grantkey.ForDomain), and asks GrantSource for exactly (identity.Sub,
// grantKey) -- never identity.Iss, never the agent id itself.
func TestResolveGrantTokenForAgent_ActiveGrant_ResolvesDomainAndAcquiresExactPair(t *testing.T) {
	...
	domainResolver := newFakeDomainResolver()
	domainResolver.agentDomains["agent-1"] = "manmanv2"
	...
	require.Equal(t, []string{"agent-1"}, domainResolver.agentCalls)
...
// proves the session-keyed entry point resolves domain via DomainForSession
// only -- DomainForAgent must never be called, even when the fake resolver
// has no agentDomains entry configured for anything at all.
func TestResolveGrantTokenForSession_ActiveGrant_ResolvesViaSessionNeverAgent(t *testing.T) {
	...
	domainResolver := newFakeDomainResolver()
	domainResolver.sessionDomains["sess-1"] = "audience_score_system"
	...
	assert.Empty(t, domainResolver.agentCalls, "DomainForAgent must never be called by the session-keyed entry point")
	require.Equal(t, []string{"sess-1"}, domainResolver.sessionCalls)

	assertForwardsToken(t, outCtx, "acquired-tok-2")
}

// TestResolveGrantTokenForAgent_NoGrantForResolvedDomain_FailsNamingDomain
// is this task's "no grant -> named domain failure" bullet: an operator
// with no active grant for the resolved domain fails, the error names that
// domain, and the outward ctx is untouched -- no fallback to any other
// credential path (NFR2).
func TestResolveGrantTokenForAgent_NoGrantForResolvedDomain_FailsNamingDomain(t *testing.T) {
	...
	assert.Contains(t, err.Error(), "manmanv2", "the failure must name the domain the operator needs to consent for")
...
// TestCrossDomainRefusal_OperatorWithOtherDomainGrant_NeverSubstituted is
// NFR2's own scenario, verbatim: an operator with an active grant for
// audience_score_system calling into a manmanv2-domain session must get a
...
// the) key requested.
func TestCrossDomainRefusal_OperatorWithOtherDomainGrant_NeverSubstituted(t *testing.T) {
	identity := mcpidentity.Identity{Iss: "https://keycloak.example.test/realms/whagent", Sub: "operator-4"}
	ctx := mcpidentity.ContextWithIdentity(context.Background(), identity)

	domainResolver := newFakeDomainResolver()
	domainResolver.sessionDomains["sess-manmanv2"] = "manmanv2"
	...
	outCtx, err := resolveGrantTokenForSession(ctx, domainResolver, grant, "sess-manmanv2")
	...
	require.Len(t, grant.calls, 1, "exactly one domain must ever be requested for this call")
	assert.Equal(t, "manmanv2", grant.calls[0].grant, "the call's own resolved domain must be requested, never the operator's other active grant's domain")
}
...
// two resolveGrantTokenForSession calls for the same (subject, domain)
...
func TestAcquireGrantToken_NoCache_ConsecutiveCallsBothHitTokenSource(t *testing.T) {
	...
	domainResolver := newFakeDomainResolver()
	domainResolver.sessionDomains["sess-1"] = "manmanv2"
	...
	_, err := resolveGrantTokenForSession(ctx, domainResolver, grant, "sess-1")
...
// TestResolveGrantTokenForAgent_DomainResolutionFails_NeverConsultsGrant
// proves a domain-resolution failure (e.g. an unknown agent id) short
// circuits before GrantSource is ever asked for anything.
func TestResolveGrantTokenForAgent_DomainResolutionFails_NeverConsultsGrant(t *testing.T) {
	identity := mcpidentity.Identity{Iss: "https://keycloak.example.test/realms/whagent", Sub: "operator-6"}
	ctx := mcpidentity.ContextWithIdentity(context.Background(), identity)

	domainResolver := newFakeDomainResolver() // agent-unknown has no entry
	grant := &fakeGrantSource{}

	outCtx, err := resolveGrantTokenForAgent(ctx, domainResolver, grant, "agent-unknown")

	require.Error(t, err)
	assert.ErrorIs(t, err, errFakeDomainNotFound)
	assert.Empty(t, grant.calls, "GrantSource must never be consulted once domain resolution has already failed")
	assert.True(t, ctx == outCtx) //nolint:staticcheck
}
...
// with a resolved domain and an active grant acquires a token via
...
func TestStartSession_EndToEnd_BrowserOAuth2Path_AcquiresAndForwardsToken(t *testing.T) {
	...
	domainResolver := newFakeDomainResolver()
	domainResolver.agentDomains["agent-e2e"] = "manmanv2"
	grant := &fakeGrantSource{
		tokenFunc: func(_ context.Context, subject, grantKey string) (*oauth2.Token, error) {
	...
	tool := &startSessionTool{client: fc, domainResolver: domainResolver, grant: grant}
...
// TestGetSession_NoGrantForResolvedDomain_ToolCallFailsWithoutReachingClient
...
func TestGetSession_NoGrantForResolvedDomain_ToolCallFailsWithoutReachingClient(t *testing.T) {
	...
	grant := &fakeGrantSource{
		tokenFunc: func(context.Context, string, string) (*oauth2.Token, error) {
			return nil, fmt.Errorf("grpcauth: no delegated grant for domain %q: %w", "manmanv2", grpcauth.ErrGrantNotFound)
		},
	}

	fc := &fakeSessionServiceClient{}
	tool := &getSessionTool{client: fc, domainResolver: domainResolver, grant: grant}
