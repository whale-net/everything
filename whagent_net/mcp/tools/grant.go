...
// a package-independent duplicate the way ../server/domain.go's
// DomainResolver return types are plain strings and error only. Depending
// on //libs/go/grpcauth directly is fine here -- unlike
// whagent_net/delegatedgrant (the composition-root-only package that
// pulls in pgstore/pgx) or whagent_net/mcpdomain (which pulls in
