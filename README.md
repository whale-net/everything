...
The repository uses Bazel's built-in testing capabilities with smart test selection based on change detection.

### Smart Test Execution

The CI pipeline uses Bazel dependency analysis to run only tests affected by changes:

```bash
# Detect which tests need to run based on changes (used in CI)
bazel run //tools:release -- tests --base-commit=main
...
**CI Testing Strategy:**
- **Pull Requests**: Compare against base branch to detect affected tests
- **Push to main**: Compare against previous commit to detect affected tests
- **Fallback**: Run all tests if change detection fails

**Benefits:**
- **Faster CI**: Only runs tests that could be affected by changes
- **Reduced noise**: No "cached test" results cluttering output
- **Precise detection**: Uses Bazel's dependency graph for accurate analysis
...
- **Change detection**: If tests don't run as expected, check the dependency analysis output in CI logs
